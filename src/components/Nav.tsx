    ...
    <AnimatePresence>
      ...
      >
        <div className='flex justify-between items-center mb-2'>
          ...
          <button className='text-white/80 hover:text-red-400 text-2xl cursor-pointer font-bold transition'
            onClick={() => setMenuOpen(false)}
          ><X /></button>
        </div>

        <div className='flex items-center gap-3 p-3 mt-3 rounded-xl bg-white/10 hover:bg-white/15 transition-all shadow-inner'>
          <div className='relative w-12 h-12 rounded-full overflow-hidden border-2 border-green-400/60 shadow-lg'>{user.image ? <Image src={user.image} alt='user' fill sizes="44px" className='object-cover rounded-full' /> : <User />}</div>
          <div>
        ...
        <div className='flex items-center gap-3 text-red-300 font-semibold cursor-pointer mt-auto hover:bg-red-500/20 p-3 rounded-lg transition-all' onClick={async()=>await signOut({callbackUrl:"/"})}>
  ...
  return (
    <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50'>
      ...
      <form  className='hidden md:flex items-center bg-white rounded-full px-4 py-2 w-1/2 max-w-lg shadow-md'>
        ...
        <input id="search" name="search" type="text" placeholder='Search groceries...' className='w-full outline-none text-gray-700 placeholder-gray-400' />
      </form>
      }
      ...
      <div className='flex items-center gap-3 md:gap-6 relative'>
        ...
        <div className='relative' ref={profileDropDown}>
          <div className='bg-white rounded-full w-11 h-11 flex items-center cursor-pointer justify-center overflow-hidden shadow-md hover:scale-105 transition-transform relative' onClick={() => setOpen(prev=>!prev)}>
          {user.image ? <Image src={user.image} alt='user' fill sizes="44px" className='object-cover rounded-full' /> : <User />}
          </div>
          ...
          <AnimatePresence>
            {open &&
              ...
              >
                ...
                <button className='flex items-center gap-2 w-full text-left px-3 py-3 hover:bg-red-50 cursor-pointer rounded-lg text-gray-700 font-medium' onClick={() => {
                  setOpen(false)
