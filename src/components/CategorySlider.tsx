...
function CategorySlider() {
  const categories = [
    ...
    {id: 4, name: "Snacks & Biscuits", icon: Cookie, color: "bg-red-100" },
    ...
    {id: 9, name: "Instant & Packaged Foods", icon: Box, color: "bg-teal-100" },
  ...
  return (
    ...
    >
      ...
      {showLeft && (
      <button className='absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white cursor-pointer shadow-lg hover:bg-green-100 rounded-full w-10 h-10 flex items-center justify-center transition-all' onClick={() => scroll("left")}><ChevronLeft className='w-6 h-6 text-green-700'/></button>
      )}
      ...
      {showRight && (
      <button className='absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white cursor-pointer shadow-lg hover:bg-green-100 rounded-full w-10 h-10 flex items-center justify-center transition-all' onClick={() => scroll("right")}><ChevronRight className='w-6 h-6 text-green-700'/></button>
      )}
    </motion.div>
  )
}
